...
const Cart = () => {
  const { cart, updateCartQuantity, removeFromCart, clearCart, addOrder } = useContext(CartContext);
  ...
  return (
    <div className="container mx-auto py-12 px-4 min-h-screen">
      <div className="relative">
        <h1 className="text-4xl font-semibold text-gray-700 mb-8 text-center">Your Cart</h1>

        {cart.length === 0 ? (
          <p className="text-lg text-gray-500 text-center">No items in your cart. Add some medicines!</p>
        ) : (
          <div className="bg-white shadow rounded-lg p-6 backdrop-blur-lg">
              ...
              {cart.map((item, index) => (
                <div key={index} className="flex justify-between items-center border-b border-gray-200 py-4">
                  ...
                  <div className="flex items-center space-x-3">
                    <FaMinusCircle
                      onClick={() => item.quantity > 1 && handleUpdateQuantity(item.name, item.quantity - 1)}
            ...
            <div className="mt-6 text-right">
              <h3 className="text-xl font-semibold text-gray-700">Total: ₹{calculateTotal()}</h3>
              <button
